...
public abstract class TopDocsCollectorContext extends QueryCollectorContext implements RescoringQueryCollectorContext {
     ...
     * -1 otherwise.
     */
   static ExecutorService executors = Executors.newFixedThreadPool(8, new ThreadFactory() {
        private final AtomicInteger counter = new AtomicInteger(1);

        @Override
        public Thread newThread(Runnable r) {
            Thread t = new Thread(r, "lookup" + "-thread-" + counter.getAndIncrement());
            t.setDaemon(false);
            return t;
        }
    });
