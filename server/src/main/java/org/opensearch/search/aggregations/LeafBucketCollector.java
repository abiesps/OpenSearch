...
public abstract class LeafBucketCollector implements LeafCollector {
     ...
     *
     * <p>The default implementation iterates doc IDs in the range and calls {@link #collect(int, long)}.
     */
    @Override
     ...
     *
     * <p>Note: The provided DocIdStream typically only holds a small subset of query matches. This method may be
     * called multiple times per segment. Like collect(int), it is guaranteed that doc IDs get collected in order,
     * ie. doc IDs are collected in order within a DocIdStream, and if called twice, all doc IDs from the second
     * DocIdStream will be greater than all doc IDs from the first DocIdStream.
     *
